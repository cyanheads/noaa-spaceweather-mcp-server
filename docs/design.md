...
**The `*1Day` probability names are deprecated in prose, not removed.** `cClass1Day`, `mClass1Day`, `xClass1Day`, and `protons1Day` read as "day one" on every forecast date, so each `.describe()` now marks it deprecated and names its date-neutral alias (#16). The fields stay in `outputSchema` until the next breaking release, since removing them changes the shape existing callers parse. `format()` renders only the date-neutral names: the service writes both names from the same upstream value, so a per-day line of the old names only repeated the numbers. A deprecated value renders on its own only when it disagrees with its alias. That keeps every distinct value in `content[]`, which is what `format-parity` checks, without printing duplicates for real data.

**Output field descriptions state the value, its unit, and its null or empty case, and nothing else.** `tools/list` loads before a client's first call, so every byte of output-schema prose is a cost on every session. Design history, feed quirks, and derivation rationale live in this document and in code comments beside the logic, not in `.describe()`. What a caller needs to read a value correctly stays in the description: which day a count or probability covers, that `null` never means level 0, that window statistics span the whole pre-reduction window, and that `summary` empties the series. `tests/tools/output-schemas.test.ts` caps the summed description bytes across every tool's success output schema (output plus enrichment fields, framework error envelope excluded) at 14,300 B, and pins each schema's shape with descriptions stripped, so prose can change without touching keys, types, `required`, or enums.
