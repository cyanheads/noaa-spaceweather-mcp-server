...
#
# The build emits platform-independent JavaScript, so keep this stage native to
# the builder instead of running Bun under cross-architecture emulation.
# ==============================================================================
...
# imports belongs in its own dependencies. The OTEL step carries the same flag
# so that install does not pull optional peers back into the runtime image.
RUN --mount=type=cache,target=/root/.bun/install/cache \
